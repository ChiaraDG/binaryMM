...
Reference to more detailed information about the method are provided at the end of this vignette.
...
The parameters from the marginal mean model have the same interpretation regardless of the dependence model used. Overall, older individuals tended to have slower recovery time than younger subjects, while females recovered quicker than males.

## Weighted Likelihood

The `mm` package allows user to add sampling weights and estimates the parameters of interest in those cases where the available sample might not be representative of the target population (i.e., survey data). This section shows how the sampling weights can be added in the `mm` syntax using the `datarand` dataframe.
...
# Functions Avalailable in the Package
